document.addEventListener("DOMContentLoaded", () => {
  const reviewCardContainer = document.getElementById("reviewCardContainer");
  const searchInput = document.getElementById("searchInput");
  const statusFilter = document.getElementById("statusFilter");
  const categoryFilter = document.getElementById("categoryFilter");
  const reviewForm = document.getElementById("reviewForm");
  const employeeName = document.getElementById("employeeName");
  const performanceRating = document.getElementById("performanceRating");
  const reviewComments = document.getElementById("reviewComments");
  const reviewStatus = document.getElementById("reviewStatus");
  const reviewCategory = document.getElementById("reviewCategory");

  const defaultImage = "https://via.placeholder.com/80x80.png?text=No+Image";

  const imageMap = {
    "Sibongile Nkosi": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSVz13n5__CJit9WIsOztZ4fdZdZmrV_RRHwg&s",
    "Lungile Moyo": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcToonwNT4zbwCyq-k-qAzXexPn6URz3gT4BxQ&s",
    "Thabo Molefe": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTMTYDFHOsEqWaxITzH0YETeYXQw6fvastO8Q&s",
    "Keshav Naidoo": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRkAkWcI0LFLuWTfiyCMiwl8TvYT_FsflDV1g&s",
    "Zanele Khumalo": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQUxarmEAWMbWnfZHQ8QVE6fZgR0fBe-63RSQ&s",
    "Sipho Zulu": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS94d9Pn8Pjsqw4yz2zRDTzZeyeXWwT26Lxqw&s",
    "Naledi Moeketsi": "data:image/jpeg;base64,",
    "Heather Johnson": "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQitWKXOGriZKh7u3b1VtWjNj8Ib7ld9rj_oA&s",
    "Chad Thompson": "data:image/jpeg;base64,",
    "Madison Clark": "data:image/jpeg;base64,"
  };

  const employeeInfoMap = {
    "Sibongile Nkosi": { position: "Software Engineer", department: "Development" },
    "Lungile Moyo": { position: "HR Manager", department: "HR" },
    "Thabo Molefe": { position: "Quality Analyst", department: "QA" },
    "Keshav Naidoo": { position: "Sales Representative", department: "Sales" },
    "Zanele Khumalo": { position: "Marketing Specialist", department: "Marketing" },
    "Sipho Zulu": { position: "UI/UX Designer", department: "Design" },
    "Naledi Moeketsi": { position: "DevOps Engineer", department: "IT" },
    "Heather Johnson": { position: "Content Strategist", department: "Marketing" },
    "Chad Thompson": { position: "Accountant", department: "Finance" },
    "Madison Clark": { position: "Customer Support Lead", department: "Support" }
  };

  let reviews = [];
  const storedReviews = localStorage.getItem("reviews");
  try {
    reviews = storedReviews ? JSON.parse(storedReviews) : null;
    if (!Array.isArray(reviews)) throw new Error("Invalid reviews data");
  } catch {
    reviews = [
      {
        name: "Sibongile Nkosi",
        rating: "Excellent",
        comments: "Outstanding problem-solving and leadership in development projects.",
        date: "2025-06-01",
        status: "Reviewed",
        category: "Leadership"
      },
      {
        name: "Lungile Moyo",
        rating: "Good",
        comments: "Strong management of HR duties and team coordination.",
        date: "2025-06-03",
        status: "Reviewed",
        category: "Management"
      },
      {
        name: "Thabo Molefe",
        rating: "Average",
        comments: "Performs well but needs to enhance testing turnaround time.",
        date: "2025-06-05",
        status: "Pending",
        category: "Productivity"
      },
      {
        name: "Keshav Naidoo",
        rating: "Excellent",
        comments: "Consistently exceeds sales targets with great client relationships.",
        date: "2025-06-07",
        status: "Reviewed",
        category: "Sales"
      },
      {
        name: "Zanele Khumalo",
        rating: "Good",
        comments: "Creative marketing strategies but on leave recently.",
        date: "2025-06-10",
        status: "Pending",
        category: "Marketing"
      },
      {
        name: "Sipho Zulu",
        rating: "Excellent",
        comments: "Strong UX design focus and collaboration.",
        date: "2025-06-12",
        status: "Reviewed",
        category: "Design"
      },
      {
        name: "Naledi Moeketsi",
        rating: "Good",
        comments: "Handles IT deployments efficiently, needs more documentation.",
        date: "2025-06-14",
        status: "Pending",
        category: "IT"
      },
      {
        name: "Heather Johnson",
        rating: "Average",
        comments: "Good content ideas, needs improvement in delivery timelines.",
        date: "2025-06-16",
        status: "Action Required",
        category: "Content Strategy"
      },
      {
        name: "Chad Thompson",
        rating: "Good",
        comments: "Reliable and accurate in monthly reports.",
        date: "2025-06-18",
        status: "Reviewed",
        category: "Finance"
      },
      {
        name: "Madison Clark",
        rating: "Average",
        comments: "Strong communicator, currently on leave during review.",
        date: "2025-06-20",
        status: "Pending",
        category: "Support"
      }
    ];
    localStorage.setItem("reviews", JSON.stringify(reviews));
  }

  function renderStars(rating) {
    const levels = { Excellent: 5, Good: 4, Average: 3, Poor: 2 };
    const stars = levels[rating] || 1;
    return `<span class="star-rating">${'★'.repeat(stars)}${'☆'.repeat(5 - stars)}</span>`;
  }

  function getStatusClass(status) {
    switch (status) {
      case "Reviewed": return "card-reviewed";
      case "Pending": return "card-pending";
      case "Action Required": return "card-action-required";
      default: return "";
    }
  }

  function validateForm() {
    let isValid = true;

    [employeeName, performanceRating, reviewStatus, reviewCategory].forEach(field => {
      field.classList.remove('is-invalid');
      const feedback = field.nextElementSibling;
      if (feedback && feedback.classList.contains('invalid-feedback')) {
        feedback.remove();
      }
    });

    if (!employeeName.value) {
      employeeName.classList.add('is-invalid');
      employeeName.insertAdjacentHTML('afterend', '<div class="invalid-feedback">Please select an employee.</div>');
      isValid = false;
    }

    if (!performanceRating.value) {
      performanceRating.classList.add('is-invalid');
      performanceRating.insertAdjacentHTML('afterend', '<div class="invalid-feedback">Please select a rating.</div>');
      isValid = false;
    }

    if (!reviewCategory.value.trim()) {
      reviewCategory.classList.add('is-invalid');
      reviewCategory.insertAdjacentHTML('afterend', '<div class="invalid-feedback">Please enter a category.</div>');
      isValid = false;
    }

    return isValid;
  }

  function getLatestReviewByEmployee(name) {
    const empReviews = reviews
      .filter(r => r.name === name)
      .sort((a, b) => new Date(b.date) - new Date(a.date));
    return empReviews.length > 0 ? empReviews[0] : null;
  }

  function fillModalWithEmployeeData(name) {
    employeeName.value = name;
    const latestReview = getLatestReviewByEmployee(name);

    if (latestReview) {
      performanceRating.value = latestReview.rating || "";
      reviewComments.value = latestReview.comments || "";
      reviewStatus.value = latestReview.status || "Pending";
      reviewCategory.value = latestReview.category || "";
    } else {
      performanceRating.value = "";
      reviewComments.value = "";
      reviewStatus.value = "Pending";
      reviewCategory.value = "";
    }

    [employeeName, performanceRating, reviewStatus, reviewCategory].forEach(field => {
      field.classList.remove('is-invalid');
      const feedback = field.nextElementSibling;
      if (feedback && feedback.classList.contains('invalid-feedback')) {
        feedback.remove();
      }
    });
  }

  function renderReviews() {
    const nameFilter = searchInput.value.toLowerCase().trim();
    const statusVal = statusFilter.value;
    const categoryVal = categoryFilter.value.toLowerCase().trim();

    reviewCardContainer.innerHTML = "";

    reviews.forEach((review, index) => {
      if (!review.name.toLowerCase().includes(nameFilter)) return;
      if (statusVal && review.status !== statusVal) return;
      if (categoryVal && !review.category.toLowerCase().includes(categoryVal)) return;

      const imgSrc = imageMap[review.name] || defaultImage;
      const position = employeeInfoMap[review.name]?.position || "Unknown Position";
      const department = employeeInfoMap[review.name]?.department || "Unknown Department";

      const card = document.createElement("div");
      card.className = `col-md-4 mb-3`;
      card.innerHTML = `
        <div class="card shadow-sm ${getStatusClass(review.status)}">
          <div class="card-body">
            <div class="d-flex align-items-center mb-3">
              <img src="${imgSrc}" alt="${review.name}" class="img-thumbnail rounded-circle me-3" width="60" height="60" />
              <div>
                <h5 class="card-title mb-0 employee-name">${review.name}</h5>
                <small class="text-muted">${position} — ${department}</small><br/>
                <small class="text-muted">${new Date(review.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</small>
              </div>
            </div>
            <p><strong>Rating:</strong> ${renderStars(review.rating)}</p>
            <p><strong>Comments:</strong> ${review.comments || 'No comments'}</p>
            <p><strong>Status:</strong> ${review.status}</p>
            <p><strong>Category:</strong> ${review.category}</p>
            <button class="btn btn-danger btn-sm mt-2" onclick="deleteReview(${index})"><i class="bi bi-trash"></i> Delete</button>
          </div>
        </div>
      `;

      const employeeNameElement = card.querySelector(".employee-name");
      employeeNameElement.addEventListener("click", () => {
        fillModalWithEmployeeData(review.name);
        const modalElement = document.getElementById("addReviewModal");
        let modal = bootstrap.Modal.getInstance(modalElement);
        if (!modal) modal = new bootstrap.Modal(modalElement);
        modal.show();
      });

      card.addEventListener("click", (e) => {
        if (e.target.closest(".employee-name") || e.target.closest("button.btn-danger")) return;
        fillModalWithEmployeeData(review.name);
        const modalElement = document.getElementById("addReviewModal");
        let modal = bootstrap.Modal.getInstance(modalElement);
        if (!modal) modal = new bootstrap.Modal(modalElement);
        modal.show();
      });

      reviewCardContainer.appendChild(card);
    });
  }

  window.deleteReview = function (index) {
    if (confirm("Are you sure you want to delete this review?")) {
      reviews.splice(index, 1);
      localStorage.setItem("reviews", JSON.stringify(reviews));
      renderReviews();
    }
  };

  reviewForm.addEventListener("submit", (e) => {
    e.preventDefault();

    if (!validateForm()) return;

    const newReview = {
      name: employeeName.value,
      rating: performanceRating.value,
      comments: reviewComments.value.trim(),
      date: new Date().toISOString().split("T")[0],
      status: reviewStatus.value,
      category: reviewCategory.value.trim()
    };

    reviews.push(newReview);
    localStorage.setItem("reviews", JSON.stringify(reviews));
    renderReviews();
    reviewForm.reset();
    employeeName.selectedIndex = 0;
    performanceRating.selectedIndex = 0;
    reviewStatus.selectedIndex = 0;

    const modalElement = document.getElementById("addReviewModal");
    let modal = bootstrap.Modal.getInstance(modalElement);
    if (!modal) modal = new bootstrap.Modal(modalElement);
    modal.hide();
  });

  searchInput.addEventListener("input", renderReviews);
  statusFilter.addEventListener("change", renderReviews);
  categoryFilter.addEventListener("input", renderReviews);

  renderReviews();
});
